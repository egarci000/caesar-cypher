...
def get_caesar_cypher(phrase, shift_factor)
  new_phrase = phrase.chars.map do |char|
    if $alphabet_in_arr.include?(char)
      if(($alphabet_in_arr.index(char) + 1) + shift_factor > 26)
        calc_expression = shift_factor - (26 + (26 - ($alphabet_in_arr.index(char) + 1) + 1))
        new_char = $alphabet_in_arr.at(calc_expression)
        char = new_char
      else
        new_char = $alphabet_in_arr.at($alphabet_in_arr.index(char) + shift_factor)
        char = new_char
      end
    else
      char = char
